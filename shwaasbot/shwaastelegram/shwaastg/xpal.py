...
shwaastgbotxpal.dhaarana(gdastras)
shwaastgbotxpal.dhaarana(smsastras)
#shwaastgbotgd = shwaastgbotxpal.gd_get_googledriver()
#sms = shwaastgbotxpal.get_sms_astra()
...
shwaastgbotxpal.logger.info("Setting up MongoEngine")
mongoengine.disconnect()
mongoengine.connect('shwaastgbot', alias='default')
...
def validate_vehicle_dict(vehicledict, new=True):
    ...
    if validation['status'] is True:
        shwaastgbotxpal.logger.info("vehicledict: " + validation['message'])
    else:
        shwaastgbotxpal.logger.error("vehicledict: " + validation['message'])
    return validation
...
def sss(invoicedict, new=True):
    ...
    if validation['status'] is True:
        shwaastgbotxpal.logger.info("invoicedict: " + validation['message'])
    else:
        shwaastgbotxpal.logger.error("invoicedict: " + validation['message'])
    return validation
...
def validate_locupdate_dict(locupdatedict, new=True):
    ...
    if validation['status'] is True:
        shwaastgbotxpal.logger.info("locupdatedict: " + validation['message'])
    else:
        shwaastgbotxpal.logger.error("locupdatedict: " + validation['message'])
    return validation
'''
...
def validate_customer_dict(customerdict, new=True):
    ...
    if validation['status'] is True:
        shwaastgbotxpal.logger.info("customerdict: " + validation['message'])
    else:
        shwaastgbotxpal.logger.error("customerdict: " + validation['message'])
    return validation
...
def validate_product_dict(productdict, new=True):
    ...
    if validation['status'] is True:
        shwaastgbotxpal.logger.info("productdict: " + validation['message'])
    else:
        shwaastgbotxpal.logger.error("productdict: " + validation['message'])
    return validation
'''
...
def validate_member_dict(memberdict, new=True):
    ...
    if validation['status'] is True:
        shwaastgbotxpal.logger.info("memberdict: " + validation['message'])
    else:
        shwaastgbotxpal.logger.error("memberdict: " + validation['message'])
    return validation
...
def validate_dutyslip_dict(dutyslipdict, new=True):
    ...
    if validation['status'] is True:
        shwaastgbotxpal.logger.info("dutyslipdict: " + validation['message'])
    else:
        shwaastgbotxpal.logger.error("dutyslipdict: " + validation['message'])
    return validation
...
def validate_assignment_dict(assignmentdict, new=True):
    ...
    if validation['status'] is True:
        shwaastgbotxpal.logger.info("assignmentdict: " + validation['message'])
    else:
        shwaastgbotxpal.logger.error("assignmentdict: " + validation['message'])
    return validation
    ...
    if "cust_id" in bookingdict.keys():
        if bookingdict['cust_id'] == "retail":
            if not bookingdict['passenger_mobile'] or bookingdict['passenger_mobile'] is None:
                ...
                validation['status'] = False
    if validation['status'] is True:
        shwaastgbotxpal.logger.info("bookingdict: " + validation['message'])
    else:
        shwaastgbotxpal.logger.error("bookingdict: " + validation['message'])
    return validation
    ...
    try:
        ...
        for key in respdict2.keys():
            ...
            if key2 != key:
                ...
                respdict.pop(key)
        shwaastgbotxpal.logger.info("Saving cust-meta as: {}".format(respdict))
        ...
        shwaastgbotxpal.logger.info("{}".format(notification))
...
def update_booking_status(booking_id, status):
    if status not in utils.validstatuses:
        shwaastgbotxpal.logger.error("Invalid status")
        return False
        ...
        if status in ['cancelled', 'new']:
            if booking.assignment is not None:
                ...
                assignment = documents.Assignment.objects.with_id(
                    booking.assignment)
                shwaastgbotxpal.logger.info("Removing booking {} from assignment {}".format(
...
def save_assignment(assignmentdict, assignment_id=None):
    ...
    if assignment_id is None:
        ...
        shwaastgbotxpal.logger.info("Created new assignment at {}".format(
    ...
    shwaastgbotxpal.logger.info("Ignoring unchanged dutyslips")
    for dutyslip in existingdutyslips:
        shwaastgbotxpal.logger.info("{}".format(dutyslip.to_json()))
        match = False
        for dutyslipdict in assignmentdict['dutyslips']:
            # shwaastgbotxpal.logger.info("{}".format(dutyslipdict))
            if dutyslip.member == dutyslipdict['member'] and dutyslip.vehicle == dutyslipdict['vehicle']:
                shwaastgbotxpal.logger.info("Unchanged {}".format(dutyslipdict))
        ...
        if match is False:
            ...
            dutyslip.delete()
    shwaastgbotxpal.logger.info("Adding the new dutyslips")
    for dutyslipdict in assignmentdict['dutyslips']:
...
def update_assignment_status(assignmentid, status):
    ...
    if assignment:
        ...
        try:
            ...
            assignment.save()
            shwaastgbotxpal.logger.info("Successfully updated assignment status")
            return True
        except Exception as e:
            shwaastgbotxpal.logger.error({}.format(str(e)))
            return False

    else:
        shwaastgbotxpal.logger.error("Assignment with that username does not exist")
        return False
...
def update_dutyslip_status(dsid, status):
    ...
    if dutyslip is None:
        shwaastgbotxpal.logger.error("No dutyslip with that id found")
        return False
...
def update_member_repayments(repayments):
    for r in repayments:
        ...
        shwaastgbotxpal.logger.info(mem.to_json())
        mem['repayments'][r['to']] += r['amount']
        mem.save()
        shwaastgbotxpal.logger.info(mem.to_json())
        mem2 = documents.Member.objects(username=r['to'])[0]
        shwaastgbotxpal.logger.info(mem2.to_json())
...
def get_xchange(xchange_id=None):
    shwaastgbotxpal.logger.info((xchange_id))
...
def create_xchange(xchangedict):
    shwaastgbotxpal.logger.info(utils.new_xchange_id())
    try:
