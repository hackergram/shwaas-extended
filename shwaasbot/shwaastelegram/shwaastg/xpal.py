...
a.save()
a.reload()
shwaasbotxpal = xetrapal.Xetrapal(a)
shwaasbotxpal.dhaarana(gdastras)
shwaasbotxpal.dhaarana(smsastras)
#shwaasbotgd = shwaasbotxpal.gd_get_googledriver()
#sms = shwaasbotxpal.get_sms_astra()
...
shwaasbotxpal.logger.info("Setting up MongoEngine")
mongoengine.disconnect()
mongoengine.connect('shwaasbot', alias='default')
...
def validate_customer_dict(customerdict, new=True):
    ...
    if validation['status'] is True:
        shwaasbotxpal.logger.info("customerdict: " + validation['message'])
    else:
        shwaasbotxpal.logger.error("customerdict: " + validation['message'])
    return validation
...
def validate_member_dict(memberdict, new=True):
    ...
    if validation['status'] is True:
        shwaasbotxpal.logger.info("memberdict: " + validation['message'])
    else:
        shwaasbotxpal.logger.error("memberdict: " + validation['message'])
    return validation
...
def update_member_repayments(repayments):
    for r in repayments:
        ...
        shwaasbotxpal.logger.info(mem.to_json())
        mem['repayments'][r['to']] += r['amount']
        mem.save()
        shwaasbotxpal.logger.info(mem.to_json())
        mem2 = documents.Member.objects(username=r['to'])[0]
        shwaasbotxpal.logger.info(mem2.to_json())
...
def get_xchange(xchange_id=None):
    shwaasbotxpal.logger.info((xchange_id))
...
def create_xchange(xchangedict):
    shwaasbotxpal.logger.info(utils.new_xchange_id())
    try:
